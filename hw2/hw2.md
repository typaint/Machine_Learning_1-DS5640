...
## tilted absolute loss function, tau=0.25
tilt_loss25 <- function(y, yhat, tau){
 tav = ifelse(y-yhat>0, 0.25*(y-yhat), (0.25-1)*(y-yhat))
 return(tav)
}
## tilted absolute loss function, tau=0.75
tilt_loss75 <- function(y, yhat, tau){
 tav = ifelse(y-yhat>0, 0.75*(y-yhat), (0.75-1)*(y-yhat))
 return(tav)
}
...
fit_lin_L1 <- function(y, x, loss=L1_loss, beta_init = c(-0.51, 0.75)) { # estimates beta1 and beta2; beta minimizes residual squares
  err <- function(beta) # calculates median L1 loss function
...
## fit simple linear model using numerical optimization
fit_lin_tilt <- function(y, x, loss=tilt_loss, beta_init = c(-0.51, 0.75), tau) { # estimates beta1 and beta2; beta minimizes residual squares
  err <- function(beta) # calculates absolute loss function
    tau*(max(loss(y,  beta[1] + beta[2]*x))) # yhat = beta[1] + beta[2]*x
...
## fit linear model - estimates betas to minimize training error
lin_beta_tilt25 <- fit_lin_tilt(y=prostate_train$lcavol,
                    x=prostate_train$lpsa,
                    loss=tilt_loss25,
                    tau=0.25)

## fit linear model - estimates betas to minimize training error
lin_beta_tilt75 <- fit_lin_tilt(y=prostate_train$lcavol,
                    x=prostate_train$lpsa,
                    loss=tilt_loss75,
                    tau=0.75)
...
## tilted absolute loss function, tau=0.25
tilt_loss25 <- function(y, yhat,tau=0.25){
 tav = ifelse(y-yhat>0, tau*(y-yhat), (tau-1)*(y-yhat))
 return(tav)
}
## tilted absolute loss function, tau=0.75
tilt_loss75 <- function(y, yhat, tau=0.75){
 tav = ifelse(y-yhat>0, tau*(y-yhat), (tau-1)*(y-yhat))
 return(tav)
}
...
## fit simple nonlinear model using numerical optimization
fit_lin_tilt_NL <- function(y, x, loss=tilt_loss, beta_init = c(-1.0, 0.0, -0.3), tau) { # estimates beta1 and beta2; beta minimizes residual squares
  err <- function(beta) # calculates absolute loss function
    tau*(max(loss(y,  beta[1] + beta[2]*exp(-beta[3]*x)))) # yhat = beta[1] + beta[2]*x
...
## fit nonlinear model - estimates betas to minimize training error
lin_beta_tilt25_NL <- fit_lin_tilt_NL(y=prostate_train$lcavol,
                    x=prostate_train$lpsa,
                    loss=tilt_loss25,
                    tau=0.25)

## fit nonlinear model - estimates betas to minimize training error
lin_beta_tilt75_NL <- fit_lin_tilt_NL(y=prostate_train$lcavol,
                    x=prostate_train$lpsa,
                    loss=tilt_loss75,
                    tau=0.75)
